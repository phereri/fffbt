...
2. the five ordered preparation steps;
...
(see Output). A step failure stops the operation — later steps are reported
`skipped`.
...
2. **Proxy** before any Instagram traffic — the proxy belongs to the account
   one-to-one and must be active before the session is touched.
...
- **Errors**: `proxy_failed`, `device_offline`.
...
Best-effort confirmation of the whole bundle. Each check that *can* run does
run; a check that cannot be performed is reported `unverified`, not failed.

- **Device profile**: visible model/locale/timezone match the profile.
- **Proxy**: external IP egress matches the proxy.
...
```json
{
  ...
  "error_message": null,
  "steps": [
    { "step": "device_profile", "status": "ok",         "detail": "model=SM-A920F locale=en_US" },
    { "step": "proxy",          "status": "ok",         "detail": "egress=US" },
    { "step": "gps",            "status": "ok",         "detail": "lat/lon within 8m" },
    { "step": "app_state",      "status": "ok",         "detail": "session restored" },
...
|---|---|---|
| `ready` | All steps passed; environment verified. | `preparing_device → publishing` |
...
| `proxy_failed` | retryable (max 3) | 2 | Proxy connection / auth error. |
...
Rationale for `needs_review` on all three: each indicates the prepared
...
- **`automation.job_events`** — one `status_changed` row for the
  ...
  step name and `StepResult` in `payload`.
